...
class Event_Management {
    ...
    async listEvent() {
        if (this.events.length !== 0) {
            ...
            this.events.forEach((event, index) => {
                console.log(chalk.bold.greenBright(`${index + 1} - ${event.title} - ${event.date} - ${event.time} - ${event.city} - ${event.stock}`));
            });
        }
    }
    // purchase Tickets ===============>>
    async purchaseTickets() {
        ...
        if (event1.stock < ticketCount) {
            ...
            console.log(chalk.italic.blueBright(`Successfully purchased ${ticketCount} tickets for ${event1.title}.`));
        }
        return;
    }
}
